...
describe("POST /api/ledger-invites/accept", () => {
  ...
  it("缺少 Origin 时返回 403", async () => {
    const request = new Request(
      "https://kuranote.test/api/ledger-invites/accept",
      {
  ...
  it("跨站 Origin 时返回 403", async () => {
    const response = await POST(createRequest({ token }, "https://evil.test"));

    expect(response.status).toBe(403);
    expect(mocks.acceptLedgerInviteService).not.toHaveBeenCalled();
  });

  it("token 无效时返回 400 和统一错误结构", async () => {
