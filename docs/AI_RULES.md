...
- 需要二次确认的破坏性操作（删除、归档等）优先复用 `useConfirmDialog()`（`src/components/providers/ConfirmDialogProvider/ConfirmDialogProvider.tsx`），不要自行维护本地 `open` state + 内联 `ConfirmationDialog`/`DeleteConfirmationDialog`。
...
- 硬性规则：随着代码规模增长，维护成本必须优先于“少改一步”的省事心态。任何可复用代码——UI 组件、业务逻辑、工具函数，也包括测试代码（test helper、mock、fixture、公共断言/渲染逻辑等）——只要在两处以上出现相同或高度相似实现，都必须优先抽取为共享实现并让各处引用，禁止各自复制一份维护。这条规则不因“只是测试代码”“影响范围小”而豁免。
- 测试文件内部或多个测试文件之间，若出现相同或高度相似的初始化 / 渲染 / 交互引导逻辑（例如多个 `render` 包装函数共享同一段"渲染后先展开/点击某个元素"的引导步骤），必须抽取为共享的 test helper 并复用，不允许为每个测试场景各自复制一份，导致后续修改需要多处同步维护。
