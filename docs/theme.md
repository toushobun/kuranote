...
- `designTokens.color.background.paper`：基础 MUI paper 色，主要用于 Dialog / Menu / Popover / Drawer 等浮层
...
- 普通业务卡片、页面内信息块优先使用 `SoftCard` / `SectionCard` / `PageCard`，背景来自 `--user-theme-card-bg`，也就是用户主题的 `palette.card`。
- 用户主题的 `palette.card` 由 `palette.surface` 派生；当前六套主题里二者没有设计差异，避免重复维护两套相同色值。
- 动态 MUI theme 中，普通 `Paper` / `Card` 的默认 `background.paper` 跟随 `palette.card`，用于没有显式使用共通卡片组件的普通 surface。
- `Dialog` / `Menu` / `Popover` / `Drawer` 等浮层组件固定使用基础 `designTokens.color.background.paper`，不套普通业务 card token，避免薰衣草 / 粉樱等主题的页面卡片色污染浮层语义。
- 半透明边框、阴影和装饰背景应优先从 theme token、用户主题 CSS variables 或 `createAlphaColor()` 等既有 helper 派生，不在组件内继续散落新的 `rgba(...)` / hex 色值。
