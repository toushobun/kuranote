...
新增 `placeholder_id uuid null`，复合外键 `(placeholder_id, ledger_id) references ledger_placeholder_member(id, ledger_id) on delete restrict`。NULL 保持匿名邀请语义。绑定一经创建不可改绑；换人要撤销旧邀请并重新创建。
...
NULL 或省略参数创建匿名邀请，非空参数创建绑定邀请。角色继续使用现有 `admin / member / viewer` 校验，不因为占位而增加角色。
...
列表 view model 明确区分三种事实来源：

| 类型                     | 展示与操作                                                                                          | 是否计入成员数       |
| ------------------------ | --------------------------------------------------------------------------------------------------- | -------------------- |
| active member            | 真实成员，沿用头像、角色及成员设置                                                                  | 是，沿用现有计数口径 |
| placeholder              | 名称 +「待邀请」；无邀请时可生成绑定邀请；有有效邀请时在该行显示「待接受邀请」、角色、复制/撤销入口 | 否                   |
| anonymous pending invite | 沿用 #431 的匿名「待接受邀请」与角色、时间，不伪造姓名                                              | 否                   |
