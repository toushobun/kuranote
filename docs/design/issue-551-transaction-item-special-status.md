...
> [!IMPORTANT]
> 本文档保留 Issue #551 当时的设计决策，仅用于历史追溯。以下关于报销 / 退款关联的数据结构、Picker、状态流转、业务净额、冻结编辑和账本开关的设计，已经由 #572 / #598 的后续实现取代，其中 #572 引入的退款多目标比例分摊又已由 #606 废弃并收敛为收入侧单目标；#574 继续负责已关联交易的受控编辑、确认式同步、解除 / 重建关联、清理流程与并发保护。当前开发不得再以本文中的 `settled_by_item_id`、报销多选、退款单选或 #572 的退款多目标模型作为实现依据；退款关联以 #606 及最新 migration / schema snapshot 为准，报销关联以 #598 及后续实现为准。
