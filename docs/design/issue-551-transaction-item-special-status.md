...
> [!IMPORTANT]
> 本文档保留 Issue #551 当时的设计决策，仅用于历史追溯。以下关于报销 / 退款关联的数据结构、Picker、状态流转、业务净额、冻结编辑和账本开关的设计，已经由 #572 / #598 的实现取代；#574 继续负责收入关联编辑与清理流程的后续规则。当前开发不得再以本文中的 `settled_by_item_id`、报销多选、退款单选等旧模型作为实现依据，应以最新 migration、schema snapshot 及上述 Issue 为准。
