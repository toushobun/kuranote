...
旧 `EmptyState` / `ErrorState` 已迁移删除。页面结果反馈统一使用 `ResultFeedback`；`LoadingState` 仅用于读取中或处理中状态。
...
`icon` 与 `illustration` 都是装饰性视觉元素，不应承载额外可访问语义。可访问语义请放在 `title` / `message` 中。

### 布局

组件自身限制内容最大宽度并居中，不依赖页面级 padding 或背景。放入普通页面、Dialog、已有卡片或 `surface="card"` 的区块状态时都应保持一致节奏。
