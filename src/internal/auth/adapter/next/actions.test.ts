...
describe("actions.test.ts", () => {
  ...
  describe("auth Next actions", () => {
    ...
    it("邮箱可用性保持现有 available / reason 结构", async () => {
      ...
      await expect(
        checkRegisterEmailAvailability("not-email"),
      ).resolves.toEqual({
        available: false,
      });
    });
    ...
    it("Google OAuth 普通异常也退回固定失败页", async () => {
      const consoleError = vi
        ...
        .mockImplementation(() => undefined);
      mocks.startGoogleAuth.mockRejectedValue(
        new Error("private oauth details"),
      );
...
describe("actions.googleOAuthNextPath.test.ts", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.headers.mockResolvedValue(
...
describe("actions.registerError.test.ts", () => {
