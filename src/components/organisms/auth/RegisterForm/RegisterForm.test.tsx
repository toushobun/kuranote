...
describe("RegisterForm.test.tsx", () => {
  beforeEach(() => {
  ...
  describe("RegisterForm", () => {
    ...
    it("邮箱失焦后提示已使用并显示登录链接，禁用获取验证码", async () => {
      ...
      expect(
        await screen.findByText("该邮箱已被注册，前往"),
      ).toBeInTheDocument();
    ...
    it("提交验证码时调用提交 action 并显示剩余次数", async () => {
      ...
      const submittedFormData = (
        props.submitOtpAction.mock.calls as unknown as Array<
          [unknown, FormData]
        >
      )[0]?.[1];
    ...
    it("可以修改注册信息并回到初始填写阶段", async () => {
      ...
      expect(screen.queryByText("邮箱格式有误")).not.toBeInTheDocument();
      expect(
        screen.queryByText("昵称最多 50 个字符。"),
      ).not.toBeInTheDocument();
    });
    ...
    it("冷却结束后可以使用锁定信息重新发送验证码", async () => {
      ...
      const resendFormData = (
        props.requestOtpAction.mock.calls as unknown as Array<
          [unknown, FormData]
        >
      )[1]?.[1];
    ...
    it("注册成功跳转前锁定 OTP 操作", async () => {
      ...
      expect(
        screen.getByRole("button", { name: "修改注册信息" }),
      ).toBeDisabled();
      expect(
...
describe("RegisterForm.turnstile.test.tsx", () => {
  afterEach(() => {
    cleanup();
...
describe("RegisterFormRedirect.test.tsx", () => {
  beforeEach(() => {
